...
bool createDirectory(const std::string& folder_path);
void deleteFilesInDirectory(const std::string& dirPath, const std::vector<std::string>& extensions);
...
int main(int argc, char **argv) {
    ...
    if (argc < 3 || argc > 5){
        cerr << endl
             << "Usage: ./mono_inertial_realsense_D455 path_to_vocabulary path_to_settings (rgbd_save_path_name)  (trajectory_file_name)"
             << endl;
        return 1;
    }

    string rgbd_file_name;
    string file_name;
    bool bFileName = false;
    string rgb_DIR = "./datasets/" + string(argv[3]) + "/rgb/";
    string depth_DIR = "./datasets/" + string(argv[3]) + "/depth/";
    string traj_DIR = "./datasets/" + string(argv[3]) + "/trajectory/";

    std::vector<std::string> extensions = {".png", ".txt"};
    deleteFilesInDirectory(rgb_DIR, extensions);
    deleteFilesInDirectory(depth_DIR, extensions);
    deleteFilesInDirectory(traj_DIR, extensions);

    bool isSucc_rgb = createDirectory(rgb_DIR);
    bool isSucc_depth = createDirectory(depth_DIR);
    bool isSucc_traj = createDirectory(traj_DIR);
    if (!isSucc_rgb || !isSucc_depth || !isSucc_traj){
        std::cerr << "Error: Failed to create required directories." << std::endl;
        throw std::runtime_error("Failed to create one or more required directories. Program will terminate.");
    }
    ...
    ORB_SLAM3::System SLAM(argv[1], argv[2], ORB_SLAM3::System::RGBD, true, 0, file_name, traj_DIR);
    ...
    while (!SLAM.isShutDown())
    {
        ...
        std::string rgb_filename = rgb_DIR + std::to_string(timestamp) + ".png";
        std::string depth_filename = depth_DIR + std::to_string(timestamp) + ".png";
        cv::Mat im_rgb;
        cv::cvtColor(im, im_rgb, cv::COLOR_BGR2RGB);
        cv::imwrite(rgb_filename, im_rgb);
...
bool createDirectory(const std::string& folder_path) {
    ...
    std::string path;
    size_t pos = 0, found;
    while ((found = folder_path.find_first_of('/', pos)) != std::string::npos) {
        path = folder_path.substr(0, found);
    ...
    if (mkdir(folder_path.c_str(), 0755) != 0 && errno != EEXIST) {
        std::cerr << "Error creating directory '" << folder_path << "': " << std::strerror(errno) << std::endl;
        return false;
    }
    return true;
}
...
bool has_extension(const std::string& filename, const std::string& ext) {
    ...
    return (pos != std::string::npos && pos == filename.length() - ext.length());
}

// Function to delete files with specific extensions in a directory
void deleteFilesInDirectory(const std::string& dirPath, const std::vector<std::string>& extensions) {
    DIR* dir = opendir(dirPath.c_str());  // Open the directory
    if (dir == nullptr) {
        std::cerr << "Unable to open directory: " << dirPath << ", Error: " << strerror(errno) << std::endl;
        return;
    }
        ...
        if (stat(filepath.c_str(), &file_stat) == 0 && S_ISREG(file_stat.st_mode)) {
            ...
            for (const auto& ext : extensions) {
                if (has_extension(filename, ext)) {
                    // Delete the file if the extension matches
                    if (remove(filepath.c_str()) != 0) {
                        std::cerr << "Failed to delete file: " << filepath << ", Error: " << strerror(errno) << std::endl;
                    }
                    break;  // Stop checking other extensions after deleting the file
                }
            }
        }
    }
